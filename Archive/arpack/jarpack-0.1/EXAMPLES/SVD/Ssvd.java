...
float [] v= new float[(250) * (25)];
float [] u= new float[(500) * (10)];
float [] workl= new float[((25*((25+8))))];
float [] workd= new float[((3*250))];
float [] s= new float[(25) * (2)];
float [] resid= new float[(250)];
...
// c        %---------------------------------------------%
// c
org.netlib.arpack.Ssaupd.ssaupd(ido,bmat,n,which,nev.val,tol,resid,0,ncv,v,0,250,iparam,0,ipntr,0,workd,0,workl,0,lworkl,info);
// c
...
rvec = true;
// c
org.netlib.arpack.Sseupd.sseupd(rvec,"All",select,0,s,0,v,0,250,sigma,bmat,n,which,nev,tol.val,resid,0,ncv,v,0,250,iparam,0,ipntr,0,workd,0,workl,0,lworkl,ierr);
// c
// c        %-----------------------------------------------%
...
for (j = 1; j <= nconv; j++) {
// c
s[(j-(1))+(1-(1)) * (25)] = (float)Math.sqrt( (double)s[(j-(1))+(1-(1)) * (25)]);
// c
// c              %-----------------------------%
...
org.netlib.blas.Scopy.scopy(m,ax,0,1,u,(1-(1))+(j-(1)) * (500),1);
temp = (1.0f/org.netlib.blas.Snrm2.snrm2(m,u,(1-(1))+(j-(1)) * (500),1));
org.netlib.blas.Sscal.sscal(m,temp,u,(1-(1))+(j-(1)) * (500),1);
// c
// c              %---------------------------%
// c              |                           |
...
// c              %---------------------------%
// c
org.netlib.blas.Saxpy.saxpy(m,(-(s[(j-(1))+(1-(1)) * (25)])),u,(1-(1))+(j-(1)) * (500),1,ax,0,1);
s[(j-(1))+(2-(1)) * (25)] = org.netlib.blas.Snrm2.snrm2(m,ax,0,1);
// c
Dummy.label("Ssvd",20);
}              //  Close for() loop. 
}
...
// c           %-------------------------------%
// c
org.netlib.arpack.Smout.smout(6,nconv,2,s,0,25,-6,"Singular values and direct residuals");
}              //  Close else.
// c
// c        %------------------------------------------%
...
// c
label9000:
   Dummy.label("Ssvd",9000);
// c
Dummy.label("Ssvd",999999);
return;
   }
} // End class.

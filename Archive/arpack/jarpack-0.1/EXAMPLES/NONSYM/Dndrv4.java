...
double [] dl= new double[(256)];
double [] du= new double[(256)];
double [] du2= new double[(256)];
...
if ((n > 256))  {
    __io_vec.clear();
  __io_vec.addElement(new String(" ERROR with _NDRV4: N is greater than MAXN "));
Util.f77write(null, __io_vec);
Dummy.go_to("Dndrv4",9000);
}
else if ((nev.val > 10))  {
    __io_vec.clear();
  __io_vec.addElement(new String(" ERROR with _NDRV4: NEV is greater than MAXNEV "));
Util.f77write(null, __io_vec);
Dummy.go_to("Dndrv4",9000);
}              // Close else if()
else if ((ncv > 25))  {
    __io_vec.clear();
  __io_vec.addElement(new String(" ERROR with _NDRV4: NCV is greater than MAXNCV "));
Util.f77write(null, __io_vec);
Dummy.go_to("Dndrv4",9000);
}              // Close else if()
bmat = "G";
...
// c     %--------------------------------------------------%
// c
dndrv4_convct.rho.val = 1.0e+1;
h = (1.0/(double)((n+1)));
s = (dndrv4_convct.rho.val/2.0);
// c
s1 = (((-((1.0/h)))-s)-((sigmar*h)/6.0));
s2 = ((2.0/h)-(((4.0e+0*sigmar)*h)/6.0));
...
dd[(j-(1))] = s2;
du[(j-(1))] = s3;
Dummy.label("Dndrv4",10);
}              //  Close for() loop. 
}
dd[(n-(1))] = s2;
// c 
org.netlib.lapack.Dgttrf.dgttrf(n,dl,0,dd,0,du,0,du2,0,ipiv,0,ierr);
if ((ierr.val != 0))  {
    __io_vec.clear();
  __io_vec.addElement(new String(" "));
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" ERROR with _gttrf in _NDRV4."));
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" "));
Util.f77write(null, __io_vec);
...
org.netlib.lapack.Dgttrs.dgttrs("N",n,1,dl,0,dd,0,du,0,du2,0,ipiv,0,workd,(ipntr[(2-(1))]-(1)),n,ierr);
if ((ierr.val != 0))  {
    __io_vec.clear();
  __io_vec.addElement(new String(" "));
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" ERROR with _gttrs in _NDRV4."));
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" "));
Util.f77write(null, __io_vec);
...
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" _NDRV4 "));
Util.f77write(null, __io_vec);
__io_vec.clear();
  __io_vec.addElement(new String(" ====== "));
Util.f77write(null, __io_vec);
...
// c
label9000:
   Dummy.label("Dndrv4",9000);
// c
Dummy.label("Dndrv4",999999);
return;
   }
} // End class.

...
/**
 * @author spupyrev 20.11.2008 This is the original algorithm found at http://code.google.com/p/graphvis/. Stored here
 *         for testing purposes, ie to see if our modified version yields the same output as the original algorithm.
 *         Multidimensional Scaling according to best distance matrix reconstruction. Based on signs of directions to
...
public class HitMDSOriginal {
	...
	private double D(double[] mat, int i, int j) {
		return mat[((i < j) ? (j - 1 + ((((pattern_length << 1) - i - 3) * i) >> 1)) : ((i == j) ? matsize
				: (i - 1 + ((((pattern_length << 1) - j - 3) * j) >> 1))))];
	}

	private void DSet(double[] mat, int i, int j, double newVal) {
		mat[((i < j) ? (j - 1 + ((((pattern_length << 1) - i - 3) * i) >> 1)) : ((i == j) ? matsize
	...
	private double corr_2() {
		...
		return pattern_distmat_var_sum * points_distmat_mono / ((p2 < e2) ? e2 : p2) - 1.0;
	}
	...
	private void data_init() {
		...
		for (int i = 0; i < pattern_length - 1; i++) {
			for (int j = i + 1; j < pattern_length; j++) {
				if (matrix_input == 0)
					DSet(pattern_distmat, i, j, distance.distance(pattern_dimension, pattern[i], pattern[j]));
				DSet(points_distmat, i, j, distance.distance(target_dim, points[i], points[j]));
			}
		}
	...
	private void mds_train(int cycles, double learning_rate) {
		...
		for (c = 0; c < cycles; c++) {
			if (++t == m) {
				t = 0;
				if (DEBUG_OUTPUT)
					System.err.println(100.0 * c / cycles + "%: " + corr_2() + " \t(r = " + 1.0 / sqrt(corr_2() + 1.0)
							+ ")");
			}
			...
			if ((diff_mixed = START_ANNEALING_RATIO * cycles - c) < 0.) {
				diff_mixed = learning_rate * (1.0 + 1.0 / (1.0 - START_ANNEALING_RATIO) * diff_mixed / cycles);
			} else {
			...
			for (k = 0; k < pattern_length; k++)
				if (k != i) {
					dtmp = distance.distance(target_dim, delta_point, points[k]);
	...
	/*
	 * move center to origin, scale data in such a way that standard deviation of dimension of largest variance is equal
	 * to 1
	 */
	...
	public double[][] evaluate(double[][] a, int destDim) {
		...
		if (DEBUG_OUTPUT) System.out.println("# corr(D,d): " + 1. / sqrt(corr_2() + 1));

...
/**
 * 
 * @see http://edndoc.esri.com/arcobjects/9.2/NET/shared/geoprocessing/
 *      spatial_analyst_tools/how_hillshade_works.htm
 * @see Burrough, P. A. and McDonell, R.A., 1998. Principles of Geographical
...
public class HillshadeAlgorithm extends MapAlgorithm {
    ...
    public void run() {
        ...
        for (Kernel k : map.kernels()) {
            double dx = (k.topRight + (2 * k.right) + k.bottomRight - (k.topLeft
                    + (2 * k.left) + k.bottomLeft)) / 8;
            double dy = (k.bottomLeft + (2 * k.bottom) + k.bottomRight - (k.topLeft
                    + (2 * k.top) + k.topRight)) / 8;
            double slopeRad = atan(z_factor * sqrt(dx * dx + (dy * dy)));
            double aspectRad = atan2(dy, -dx);
            double shading = (cos(zenithRad) * cos(slopeRad) + (sin(zenithRad)
                    * sin(slopeRad) * cos(azimuthRad - aspectRad)));

...
public abstract class Collect<E> implements Iterable<Each<E>> {
	...
	public static <E> Collect<E> fromCollection(Iterable<E> elements) {
		return new CollectFromCollection<E>(elements);
	}
	...
	private static class CollectFromArray<E> extends Collect<E> implements
			Iterator<Each<E>> {
	...
	private static class CollectFromCollection<E> extends Collect<E> implements
			Iterator<Each<E>> {
		...
		public CollectFromCollection(Iterable<E> elements) {
			this.state = NULL;
		...
		public E[] resultArray() {
			return resultArray((Class<? extends E>) getResult().iterator()
					.next().getClass());
		}
		...
		public E[] resultArray(Class<? extends E> type) {
			return getResult().toArray(
					(E[]) Array.newInstance(type, result.size()));
		}
